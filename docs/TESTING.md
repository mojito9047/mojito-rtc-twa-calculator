...
| `test_autostart.py` | `autostart.py`: asking at install, moving auto-start to the new version without asking, replacing a hand-made shortcut, leaving other programs' shortcuts alone, turning it off, the `autostart.bat` menu; and on Windows a real shortcut, minimised, read back. |
